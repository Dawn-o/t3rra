# Fetch.ai x Internet Computer (Motoko Backend)

This repository is a Motoko-based remake of the [Fetch.ai + ICP Integration](https://github.com/fetchai/innovation-lab-examples/tree/main/web3/internet-computer), which originally used Azle for the backend. This version replaces the Azle backend with a Motoko implementation.
...
└── ic/                       # Internet Computer backend (Motoko canister)
    ├── src/
...
The ICP component (`ic/src/backend/main.mo`) implements a dummy HTTP server with the following endpoints:
